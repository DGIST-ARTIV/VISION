...
def image_callback(msg : Image):
    ...
    for i in range(0, 4):
        if exist_pred[0][i] > 0.5:
            ...
            if len(x_list)>=5:
                ...
                if len(x_list) >= 4:
                    f1, lane_img, img = draw_polynomial_regression_lane(x_list, y_list, img, color_choice[i])
    ...
    if f_left != None and f_right != None:
        ...
        draw_poly = np.array([list(zip(x_list, y_list))], np.int32)
        lane_img2 = cv2.polylines(lane_img2, np.asarray(draw_poly), False, (0, 100, 255), 3)
