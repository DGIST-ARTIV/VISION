...
def centerline(img, seg_pred, exist_pred):
...
def image_callback(msg : Image):
    ...
    if re_eval_sign(points) == True or frame_100 == 50:
        ...
        # color_choice = centerline(hls, seg_pred, exist_pred)
        hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
        color_choice = centerline(hsv, seg_pred, exist_pred)
    ...
    for i in range(0, 4):
        if exist_pred[0][i] > 0.5:
            ...
            if len(x_list)>=5:
                ...
                if len(x_list) >= 4:
                    print("entenenenneeeeeeeeeeeeeeeee")
                    lane_img, img, f1 = draw_polynomial_regression_lane(x_list, y_list, img, color_choice[i])
                    ...
                    if i == 1:
                        ...
                        f_right = f1
    if f_left != None and f_right != None:
        f_center = (f_left+f_right)/2
        x_list = np.array([300,350,400,450,500])
        y_list = np.polyval(f1, x_list)
        centerPt = [(x_list[i], y_list[i]) for i in range(len(x_list))]  
        img = cv2.polylines(img, np.asarray(centerPt), False, (0, 100, 255), 3)
    # cv2.imshow("img", img)
    # cv2.waitKey(1)
...
if len(x_left)>=4 and len(x_right) >=4 and len(y_left)>=4 and len(y_right)>=4:
    ...
    centerPt = [(((x_left[i] + x_right[i])/2), ((y_left[i] + y_right[i])/2)) for i in range(min([len(x_left),  len(x_right),  len(y_right),  len(y_left)]))]
    '''
