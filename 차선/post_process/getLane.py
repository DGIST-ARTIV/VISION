...
def prob2lines_CULane_make(seg_pred, exist, resize_shape=None, smooth=True, y_px_gap=10, pts=None, thresh=0.25):
    """
    Arguments:
    ----------
    seg_pred: np.array size (5, h, w)
    ...
    seg_pred = np.array(np.transpose(seg_pred, (1, 2, 0)))
    ...
    for i in range(4):
        ...
        if exist[i] > 0:
            coords = getLane_CULane(prob_map, y_px_gap, pts, thresh, resize_shape)
            coordinates.append([[int(coords[j]), H - 1 - j * y_px_gap] for j in range(pts) if coords[j] > 0])
            pass
        if exist[i] == 0:
            coordinates.append([None])
    return coordinates
    #print(coords)
    # cv2.imshow("before_lane_img", before_lane_img)
    # cv2.imshow("after_lane_img", after_lane_img)

    # print(coordinates)
    """
    for n, x in enumerate(coordinates):
        for i in range(len(x) - 2):
            if exist_pred[0, n] > 0.5:
                cv2.line(after_lane_img, (int(x[i][0]), int(x[i][1])), (int(x[i + 1][0]), int(x[i + 1][1])), 128, 6)
    """
    # cv2.imshow("draw_lane_img", after_lane_img)
