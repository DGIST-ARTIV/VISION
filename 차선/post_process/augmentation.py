...
with open(path) as output:
    ...
    for idx, li in enumerate(str_out):
        ...
        image = augment_and_show(aug, x, "rain")
        new_file_path = "./"+root+'/'+"aug"+file_path[:file_path.find('/',7)]
        new_file_path_ = '/'+"aug"+file_path[:file_path.find('/',7)] # except root
        ...
        with open("./"+root+"/list/train.txt", 'w') as output:
            output.write(aug_jpg_file_)
