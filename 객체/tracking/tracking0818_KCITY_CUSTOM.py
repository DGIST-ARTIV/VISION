...
class Queue_for_tracking:
    ...
    def make_rosmsg(self):
        result = self._queue[0]
        detection_results = Int16MultiArray()
        if len(result) == 0:
            return None
        for i, obj in enumerate(result):
            xmin = obj.center_x - (obj.width//2)
            ymin = obj.center_y - (obj.height//2)
            xmax = obj.center_x + (obj.width//2)
            ymax = obj.center_y + (obj.height//2)
            temp = obj.id
            if obj.id == None:
                temp = -1
            detection_results.layout.dim.append(MultiArrayDimension())
            detection_results.layout.dim[i].label = "object" + str(i)
            detection_results.layout.dim[i].size = 7
            detection_results.data = [0] * 7 * len(result)
            detection_results.data[i * 6] = KCITY_CUSTOM_LIST.index(obj.clss)
            detection_results.data[i * 6 + 1] = temp
            detection_results.data[i * 6 + 2] = obj.conf
            detection_results.data[i * 6 + 3] = xmin
            detection_results.data[i * 6 + 4] = ymin
            detection_results.data[i * 6 + 5] = xmax
            detection_results.data[i * 6 + 6] = ymax
...
class obj:
    def __init__(self, xmin, xmax, ymin, ymax, clss, conf):
        ...
        self.clss = KCITY_CUSTOM_LIST[clss]
        self.conf = conf
        self.id = None
        self.isReal = True
...
def Bbox_callback(msg: Int16MultiArray):
    global pub0, pub1, pub2, pub3, pub4, pub5, pub6, pub7, pub8, pub9, pub10, pub11, pub12, pub13
    now = time.time()
    data = msg.data
    list_0 = []
    list_1 = []
    ...
    msg0 = Queue_0.execute(sorted(list_0))
    msg1 = Queue_1.execute(sorted(list_1))
    msg2 = Queue_2.execute(sorted(list_2))
    msg3 = Queue_3.execute(sorted(list_3))
    msg4 = Queue_4.execute(sorted(list_4))
    msg5 = Queue_5.execute(sorted(list_5))
    msg6 = Queue_6.execute(sorted(list_6))
    msg7 = Queue_7.execute(sorted(list_7))
    msg8 = Queue_8.execute(sorted(list_8))
    msg9 = Queue_9.execute(sorted(list_9))
    msg10 = Queue_10.execute(sorted(list_10))
    msg11 = Queue_11.execute(sorted(list_11))
    msg12 = Queue_12.execute(sorted(list_12))
    msg13 = Queue_13.execute(sorted(list_13))

    gc.collect()
    total_pub(msg0, msg1, msg2, msg3, msg4, msg5, msg6, msg7, msg8, msg9, msg10, msg11, msg12, msg13)
    print(time.time()-now)
    print("----------------------------------------------------------------------------")
...
def total_pub(a, b, c, d, e, f, g, h, i, j, k, l, m, n):
    global pub0, pub1, pub2, pub3, pub4, pub5, pub6, pub7, pub8, pub9, pub10, pub11, pub12, pub13
    print("fuck")
    if a != None:
        pub0.publish(a)
...
def main():
    ...
    pub0 = node.create_publisher(Int16MultiArray, '/tracking/car')
    pub1 = node.create_publisher(Int16MultiArray, '/tracking/crossWalk_sign')
    pub2 = node.create_publisher(Int16MultiArray, '/tracking/bicycle_sign')
    pub3 = node.create_publisher(Int16MultiArray, '/tracking/bust_sign')
    pub4 = node.create_publisher(Int16MultiArray, '/tracking/construction_sign')
    pub5 = node.create_publisher(Int16MultiArray, '/tracking/parking_sign')
    pub6 = node.create_publisher(Int16MultiArray, '/tracking/kidSafeSero_sign')
    pub7 = node.create_publisher(Int16MultiArray, '/tracking/busArrowDown_sign')
    pub8 = node.create_publisher(Int16MultiArray, '/tracking/trafficLightRedYellow')
    pub9 = node.create_publisher(Int16MultiArray, '/tracking/trafficLightGreenLeft')
    pub10 = node.create_publisher(Int16MultiArray, '/tracking/trafficLightYellow')
    pub11 = node.create_publisher(Int16MultiArray, '/tracking/trafficGreen')
    pub12 = node.create_publisher(Int16MultiArray, '/tracking/trafficLightRed')
    pub13 = node.create_publisher(Int16MultiArray, '/tracking/trafficLightRedLeft')
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
